...
class EditProfileModal extends Component {
  ...
  getUserInfor = () => {
    ...
    if (userInfo) {
      this.setState({
        ...
        avatar: imageBase64 ? imageBase64 : null,
    ...
    for (let i = 0; i < arrCheck.length; i++) {
      if (!this.state[arrCheck[i]]) {
        ...
        if (this.props.language == "en") {
          toast.error("Missing parameter: " + arrCheck[i]);
        } else {
          toast.error("Chưa nhập đủ thông tin: " + arrCheck[i]);
        }
        break;
      }
    }
  ...
  handleSaveUser = async () => {
    ...
    if (!this.state.isValidAddress || !this.state.isValidFirstName || !this.state.isValidLastName || !this.state.isValidPhoneNumber) return
